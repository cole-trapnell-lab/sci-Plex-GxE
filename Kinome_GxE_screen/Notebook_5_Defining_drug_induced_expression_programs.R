...
for(cell_type in names(treatment_dose_dependent_diff_test.list)){
  ...
  treatment_dose_dependent_diff_test.list[[cell_type]] <- treatment_dose_dependent_diff_test.list[[cell_type]][!is.na(treatment_dose_dependent_diff_test.list[[cell_type]]$p_value),]
  treatment_dose_dependent_diff_test.list[[cell_type]]$q_value <- p.adjust(treatment_dose_dependent_diff_test.list[[cell_type]]$p_value, method = "BH")
  
}

### Save diff test
### Note: This was collapsed across all lines although the anlaysis if separate below
treatment_dose_dependent_diff_test_results <- do.call("rbind",treatment_dose_dependent_diff_test.list)
write.table(treatment_dose_dependent_diff_test_results, "RTK_inhibitor_treatment_dose_dependent_diff_test_results_related_to_Figure_4.txt", 
            sep = "\t",
            quote = FALSE,
            row.names = FALSE,
            col.names = TRUE)
