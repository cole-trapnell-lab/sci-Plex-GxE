...
plot_percent_cells_positive(HPRT1_plot_subset[,colData(HPRT1_plot_subset)$CRISPR_hash == "CRISPRi"],
  ...
  ggsave("gRNA_QC_plots/HPRT1_levels_across_CRISPRi.png", 
...
plot_percent_cells_positive(HPRT1_plot_subset[,colData(HPRT1_plot_subset)$CRISPR_hash == "CRISPRa"],
  ...
  theme(text = element_text(size = 6),
        ...
        legend.position = "none",
        strip.text.x = element_blank()) +
  ggsave("gRNA_QC_plots/HPRT1_levels_across_CRISPRa.png", 
         dpi = 600, height = 1.25, width = 0.8)
...
colData(HPRT1_cds) %>%
  ...
  distinct() %>%
  ungroup() %>%
  #mutate(CRISPR = factor(CRISPR_hash, levels = c("CRISPRi","CRISPRa"))) %>% 
  ggplot() +
  geom_tile(aes(x = gene_id, y = dose, fill = mean_proliferation_index)) +
  #facet_wrap(~CRISPR_hash, scales = "free") +
  ...
  ggsave("Heatmaps/HPRT1_proliferation_heatmap.png",
...
ggplot(colData(HPRT1_cds) %>%as.data.frame(),
  ...
  ggsave("gRNA_QC_plots/CRISPR_barnyard_results.png", 
