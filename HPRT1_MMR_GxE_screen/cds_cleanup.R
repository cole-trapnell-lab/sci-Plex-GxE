...
suppressPackageStartupMessages({
  source("~/Documents/github_repos/sci-plex/bin/dose_response.R")
  source("~/Documents/github_repos/sci-plex/bin/cell_cycle.R")
  source("~/Documents/github_repos/sci-plex/bin/viability.R")
...
setwd("~/Dropbox (Cole Trapnell's Lab)/Shared Slides and Figures/McFalineJL/sci-chem-kinome/sci-Plex-6-HPRT1-MMR")

cds <- readRDS("cds_w_hashData.RDS")
...
### Incorporate sgRNA enrichment results
gRNA_enrichment_results <- readRDS("sgRNA_sequences_table_out.rds")

# sgRNA metadata
CRISPR_library <- read.table("MMR_HPRT1_libraries.txt", header = T)
...
ggplot(gRNA_proportions, aes(x = ratio1, y= ratio2, color = log(total))) +
  ...
  ggsave("gRNA_QC_plots/CRISPR_MMR-HPRT1_gRNA_proportions_per_cell.png", 
...
ggplot(gRNA_proportions, aes(x = log10(top_to_second_best_ratio))) +
  ...
  monocle3:::monocle_theme_opts() +
  theme(text = element_text(size = 6)) +
  ggsave("gRNA_QC_plots/CRISPR_MMR-HPRT1_gRNA_top_to_second_best_ratio.png", 
...
ggplot(gRNA_proportions[gRNA_proportions$ratio > 0,], aes(x = ratio, y= log10(total))) +
  ...
  theme(text = element_text(size = 6),
        legend.key.width = unit(0.1, "in"),
        legend.key.height = unit(0.1, "in")) +
  ggsave("gRNA_QC_plots/CRISPR_MMR-HPRT1_gRNA_ratios_per_total.png", 
         width = 2, height = 1.5, dpi = 600)
